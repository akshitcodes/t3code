...
export interface ComposerThreadDraftState {
   ...
   * historical `ProviderDriverKind` literal (`codex` / `claudeAgent` / `copilot` /
   * `cursor` / `opencode`) also satisfies the `ProviderInstanceId` slug pattern, so
...
function normalizeProviderModelOptions(
  ...
  const result: ProviderOptionSelectionsByProvider = {};
  for (const providerKey of ["codex", "claudeAgent", "copilot", "cursor", "opencode"] as const) {
...
function legacyToModelSelectionByProvider(
  ...
  if (modelOptions) {
    for (const provider of ["codex", "claudeAgent", "copilot", "cursor", "opencode"] as const) {
